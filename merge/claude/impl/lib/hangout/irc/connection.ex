defmodule Hangout.IRC.Connection do
  ...
  def init({ref, transport, _opts}) do
    ...
    transport.setopts(socket, active: :once, packet: :line, buffer: 16_384)
  ...
  def handle_info({:tcp, socket, data}, %{socket: socket} = state) do
    ...
    handle_line(String.trim_trailing(to_string(data), "\r\n"), state)
  end
  ...
  def handle_info({:channel_created, _channel, _token}, state) do
    # Handled during join flow, not here
    {:noreply, state}
  end
  ...
  def terminate(_reason, state) do
    # Part all channels and unregister nick
  ...
  defp dispatch("CAP", _params, state) do
    # Silently accept CAP for client compat
    {:noreply, state}
  end
  ...
  defp dispatch("NICK", [nick], state) do
    cond do
      ...
      state.nick == nil ->
        # First NICK during registration
  ...
  defp dispatch("PASS", _params, state) do
    # No password required, silently accept
    {:noreply, state}
  end

  # Post-registration commands — guard on registered
  ...
  defp dispatch("JOIN", [channels_str | _], state) do
    channels = String.split(channels_str, ",")
        ...
        cond do
          ...
          true ->
            case ChannelSupervisor.ensure_channel(channel_name) do
              {:ok, _pid} ->
                participant = Participant.new(state.nick, :irc, self(),
                  user: state.user,
                ...
                case ChannelServer.join(channel_name, participant) do
                  {:ok, info} ->
                    ...
                    # Send capability URL notice if this is the creator (first member)
                    if length(info.members) == 1 do
                      # The mod token was broadcast on creation, but we need to get it
                      # from the channel_created event that was broadcast
                      send_line(acc, Parser.server_msg("NOTICE", channel_name, "You are the room creator. Use MODAUTH <token> to claim moderation."))
                    end
  ...
  defp dispatch("KICK", [channel_name, target | rest], state) do
    ...
    case ChannelServer.kick(channel_name, state.nick, target, reason) do
      ...
      {:error, :chanop_needed} ->
        send_line(state, Parser.numeric(482, state.nick, channel_name, "You're not channel operator"))
      {:error, :not_in_channel} ->
        send_line(state, Parser.numeric(441, state.nick, "#{target} #{channel_name}", "They aren't on that channel"))
    end
    ...
    case ChannelServer.who(channel_name) do
      {:ok, who_list} ->
        for entry <- who_list do
          ...
          send_line(state, ":hangout 352 #{state.nick} #{channel_name} #{entry.user} hangout hangout #{entry.nick} H#{prefix} :0 #{entry.realname}\r\n")
        end
  ...
  defp dispatch("WHOIS", [nick | _], state) do
    # Search in all channels the user is in
    found =
    ...
    case found do
      ...
      info ->
        send_line(state, ":hangout 311 #{state.nick} #{info.nick} #{info.user} hangout * :#{info.realname}\r\n")
  ...
  defp dispatch("BOT", _params, state) do
    ...
    # Mark as bot in all joined channels
    for channel <- state.channels do
      try do
    ...
    case Integer.parse(seconds_str) do
      {seconds, ""} ->
        case ChannelServer.set_ttl(channel_name, state.nick, seconds) do
          :ok ->
            send_line(state, Parser.server_msg("NOTICE", channel_name, "Room TTL set to #{seconds} seconds"))
  ...
  defp maybe_complete_registration(%{nick: nick, user: user} = state)
    ...
    send_line(state, ":hangout 005 #{nick} CHANTYPES=# NICKLEN=16 CHANNELLEN=48 :are supported by this server\r\n")
