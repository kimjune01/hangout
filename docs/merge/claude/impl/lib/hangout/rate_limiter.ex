defmodule Hangout.RateLimiter do
  ...
  """
  def new(max_tokens, refill_rate, refill_interval_ms) do
    %__MODULE__{
  ...
  @doc """
  Returns the default message rate limiter from config.
  """
  def default_message_limiter do
    {rate, interval_ms} = Application.get_env(:hangout, :message_rate_limit, {5, 10_000})
    burst = Application.get_env(:hangout, :message_burst, 10)
    new(burst, rate, interval_ms)
  end
